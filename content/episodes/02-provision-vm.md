...
- Understand alternative compute options on Sigma2 HPC clusters
```
...
For large-scale training or running multiple experiments in parallel, you can use Sigma2 national HPC clusters instead of (or in addition to) NAIC Orchestrator VMs.

### Available Clusters

| Cluster | GPUs | Best For |
|---------|------|----------|
| **Saga** | NVIDIA P100 (16 GB) | Development, small-scale training |
| **Betzy** | NVIDIA A100 (40 GB) | Large-scale training, multi-GPU |
| **LUMI** | AMD MI250X | Very large workloads (EuroHPC) |

### Getting Access

1. Apply for a compute allocation at [my.sigma2.no](https://www.sigma2.no/user-portal)
2. Follow the [Sigma2 documentation](https://documentation.sigma2.no) for SSH setup
3. Use SLURM job scripts to submit training jobs

### Example SLURM Job Script

```bash
#!/bin/bash
#SBATCH --job-name=ais-gnn
...
Submit with `sbatch job_script.sh` and monitor with `squeue -u $USER`.

```{keypoints}
- The NAIC Orchestrator provides GPU-enabled VMs pre-configured for AI workloads
- Always verify GPU availability with `nvidia-smi` after provisioning
- Set correct SSH key permissions (`chmod 600`) to avoid connection issues
- Sigma2 clusters (Saga, Betzy, LUMI) are available for large-scale training
- The `vm-init.sh` script handles system package installation and CUDA configuration
- Use SLURM job scripts for batch training on HPC clusters
```
